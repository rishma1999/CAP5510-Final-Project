...
The dataset contains microarray gene-expression measurements from 72 leukemia patients, including 38 training samples and 34 independent test samples, each labeled as ALL (0) or AML (1). Although the original data includes thousands of gene features and additional metadata such as Gene Description, Gene Accession Number, and probe “call” indicators, all non-expression fields were removed during preprocessing. The gene matrices were then aligned and transposed so that each row corresponds to a patient and each column to a gene. Following data cleaning and feature-ranking procedures, the dimensionality was reduced to the 50, 100 and 250 most informative genes, which were used as the final input set for model training and analysis.
